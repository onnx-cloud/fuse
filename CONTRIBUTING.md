...
**Development environment:** We use a project virtualenv at `.venv`. The `Makefile` prefers the `.venv` Python when present and `make test` now runs `scripts/ensure-venv.sh` to help you avoid forgetting to activate the env. You can check activation manually with:

```
./scripts/ensure-venv.sh
```

Run `./scripts/ensure-venv.sh --create` to create a `.venv` if you don't have one yet.
...
- Use `stable_name_allocator` fixture for deterministic fn names in lowering tests. This prevents flakiness caused by global counters or varying dict iteration orders.
...
If you add a test that depends on specific fn names or serialized bytes, add a snapshot assertion (e.g., in `tests/name_allocator/`) and explain in the test comment why the snapshot should remain stable.
