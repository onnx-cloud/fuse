...
- Properties
  ...
  - `onnx:domain`  (Node → domain IRI or literal)
  - `onnx:opType`  (Node → operator name, e.g., "Conv")
  - `onnx:inputs` (Node → literal list of input names, compact summary)
  - `onnx:outputs` (Node → literal list of output names, compact summary)
  - `onnx:name`    (Node or Initializer → literal name)
...
3. **Node URIs**
   - Use `:node/<stable-name>` (stable name allocator from GraphContext), e.g. `my:GPT a onnx:Node ; onnx:domain my:ai.onnx ; ... .`
...
- Avoid random blank-node labels. If blank nodes are used, generate labels from stable components (model hash, node index, role).
