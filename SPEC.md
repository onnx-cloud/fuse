...
* **Single node per import**: groups all variants
...
5. **Multi-node graph building**: wire outputs → inputs across fused models
...
- `@training { ... }` — a module-level metadata block describing training configuration. Supports both flat kv pairs and small nested blocks (e.g., `optimizer: { type = adamw, lr = 0.001 }`). The transformer produces a `meta` node `{ name: "fuse.training", value: {...} }`.
