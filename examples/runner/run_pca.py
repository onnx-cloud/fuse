...
# Runner assumes ONNX model already exists at onnx/cookbook/pca_recon.onnx
model_path = Path("onnx/cookbook/pca_recon.onnx")
...
model_path = outdir / "pca_recon.onnx"
if model_path.is_dir():
    found = next(model_path.rglob("*.onnx"), None)
    if not found:
...
print("Reconstruction for sample", sample, "=>", outs[0])
