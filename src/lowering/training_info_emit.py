...
def emit_training_info(ctx, grad_summary: Dict[str, List]) -> None:
    ...
    if explicit_alg is not None:
        ...
        try:
            getattr(ctx, "opset", None) or getattr(ctx, "opset", 18)
            int(ctx.extra_opsets.get(TRAINING_DOMAIN, 1))
    ...
    # Validate loss_binding entries reference actual algorithm outputs
    import logging
    logger = logging.getLogger(__name__)
    try:
        lb = getattr(ti, "loss_binding", None)
        if lb:
            # Handle different loss_binding implementations (map, repeated field, or list)
            try:
                # Treat as dict-like or iterable of KV pairs
                items = lb.items() if hasattr(lb, "items") else (
                    [(e.key, e.value) for e in lb] if hasattr(lb, "__iter__") else []
                )
                for key, value in items:
                    if value not in alg_outputs:
                        logger.warning(
                            f"Loss binding '{key}' → '{value}' not found in algorithm outputs. "
                            f"Available outputs: {list(alg_outputs.keys())}"
                        )
            except Exception:
                # If we can't iterate, skip validation silently
                pass
    except Exception:
        pass
