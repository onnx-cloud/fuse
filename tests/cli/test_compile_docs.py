...
def test_compile_docs_invokes_cmd_docs(monkeypatch):
    ...
    rc, out = _capture_stdout(main, ['compile', '-f', 'm.fuse', '--docs'])
    assert rc == 0
    assert 'out1' in out
    assert called['args'][0] == ['/tmp/fake_model.onnx']
    # docs invoked with md/ttl/dot/ast True
    assert called['args'][1].get('md') is True
    assert called['args'][1].get('ttl') is True
    assert called['args'][1].get('dot') is True
    assert called['args'][1].get('ast') is True
